    ...
    pub fn dequip(&mut self, messages: &mut Messages) {
        ...
        if let Some(ref mut equipment) = self.equipment {
            if equipment.equipped {
                ...
                messages.add(format!("Dequipped {} from {}.", self.name, equipment.slot), LIGHT_YELLOW);
            }
        } else {
...
fn cast_confuse(
    ...
    if let Some(monster_id) = monster_id {
        ...
        game.messages.add(
            format!("The eyes of {} look vacant, as he starts to stumble around!", objects[monster_id].name),
            LIGHT_GREEN
        );
        UseResult::UsedUp
    } else {
...
fn menu<T: AsRef<str>>(header: &str, options: &[T], width: i32, root: &mut Root) -> Option<usize> {
    debug_assert!(options.len() <= 26, "Cannot have a menu with more than 26 options.");
...
/// move by the given amount, if the destination is nto blocked
...
fn make_map(objects: &mut Vec<Object>, level: u32) -> Map {
    ...
    for _ in 0..MAX_ROOMS {
        ...
        if !failed {
            ...
            if rooms.is_empty() {
                // this is the first room, where the palyer starts at
...
Defense: {}",
        ...
        (Key { code: Home ,    .. }, _, true) => { player_move_or_attack(-1, -1, game, objects); TookTurn },
        (Key { code: PageUp,   .. }, _, true) => { player_move_or_attack( 1, -1, game, objects); TookTurn },
        (Key { code: End,      .. }, _, true) => { player_move_or_attack(-1,  1, game, objects); TookTurn },
        (Key { code: PageDown, .. }, _, true) => { player_move_or_attack( 1,  1, game, objects); TookTurn },
...
fn new_game(tcod: &mut Tcod) -> (Game, Vec<Object>) {
    ...
    let mut player = Object::new(0, 0, '@', "palyer", WHITE, true);
    player.alive = true;
